...
public class CreationU_RFO_Controller {
	
	EntityManagerFactory factory;
	static String errors = "";
	static Logger logger = LogManager.getLogger("RFOCreation");
	...
	static String toEmail = System.getProperty("CreationRFO_toEmail");
	
	public CreationU_RFO_Controller() {
	...
	public static void sendEmailRequest(ArrayList <INT22_SND> arrayReq) {
	...
	public static void sendEmailResponse(INT22_TRAX response) {
		if(toEmail == null || toEmail.trim().isEmpty()) {
			logger.severe("Email address (toEmail) is not configured. Please check the system properties.");
			return;
		}
		try {
			StringBuilder responses = new StringBuilder();
				responses.append("WO: ").append(response.getTransaction()).append(",");
	...
	public static void sendEmailOpsLine(String Operation, INT22_TRAX order, OpsLineEmail opsLineEmails) {
		if (toEmail == null || toEmail.trim().isEmpty()) {
		...
		try {
			...
			if(opsLineEmails.getFlag() != null && !opsLineEmails.getFlag().isEmpty() && (opsLineEmails.getFlag().equalsIgnoreCase("Y") || opsLineEmails.getFlag().equalsIgnoreCase("I"))) {
				email.setSubject("Failure to update INT 22 WO: " + order.getTraxWoNumber() + " SVO: " + order.getSapSvo());
				email.setMsg("WO: " + order.getTraxWoNumber() + " SVO: " + order.getSapSvo() + ", Date & Time of Transaction: " + date +  ", Error Code: " + order.getErrorCode() + ", Remarks: " + order.getRemarks());				
			} else {
				email.setSubject("Failure to update INT 22 WO: " + order.getTraxWoNumber() + " SVO: " + order.getSapSvo());
				email.setMsg("WO: " + order.getTraxWoNumber() + " SVO: " + order.getSapSvo() + ", Date & Time of Transaction: " + date +  ", Error Code: " + order.getErrorCode() + ", Remarks: " + order.getRemarks());			
			}
