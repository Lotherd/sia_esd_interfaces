...
public class Service {
	...
	public Response setOpsLine(@QueryParam("opsLine") String opsLine, @QueryParam("email") String email) {
		...
		TECO_Handling_Data data = new TECO_Handling_Data();
		...
		try {
			...
			TECO_Handling_Controller.sendEmailService(executed);
	...
	public Response markTransaction(INT15_TRAX input) {
		String executed = "OK";
		TECO_Handling_Data data = new TECO_Handling_Data("mark");
		try {
			JAXBContext jc = JAXBContext.newInstance(INT15_TRAX.class);
			...
			logger.severe(e.toString());
			TECO_Handling_Controller.addError(e.toString());
				OpsLineEmail opsLineEmail = data.getOpsLineStaffName(input.getRFO_NO());
				TECO_Handling_Controller.sendEmailOpsLine(null, input, opsLineEmail);
		} finally {
			try {
				if(data.getCon() != null && !data.getCon().isClosed())
					data.getCon().close();
