...
public class ServiceClient {
    ...
    public boolean callSap(MT_TRAX_SND_I21_4121_REQ data) {
        final int MAX_ATTEMPTS = Integer.parseInt(System.getProperty("PR_MAX_ATTEMPTS", "-1"));
        final long RETRY_INTERVAL = Long.parseLong(System.getProperty("PR_RETRY_INTERVAL", "180")) * 1000;
        ...
        while (MAX_ATTEMPTS == -1 || attempt <= MAX_ATTEMPTS) {
            ...
            try {
                String url = System.getProperty("PR_url");
                if (url == null) {
    ...
    private void logXmlContent(MT_TRAX_SND_I21_4121_REQ data, int attempt) {
        try {
    ...
    private Client getRestSSLClient(String accept, String contentType) {
        ...
        try {
            ...
            if (contentType != null) client.property("Content-Type", contentType);
            if (accept != null) client.property("accept", accept);
        } catch (Exception exc) {
            exc.printStackTrace();
        }
        return client;
