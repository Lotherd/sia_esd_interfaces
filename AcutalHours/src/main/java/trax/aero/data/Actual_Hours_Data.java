...
public class Actual_Hours_Data {
	...
	public ArrayList<INT31_SND> getActualHours() throws Exception{
		...
		String sqlActualHR = "SELECT DISTINCT W.RFO_NO, WTI.OPS_NO, WA.EMPLOYEE, ABS(WA.HOURS * 60 + WA.MINUTES) AS TOTAL_MINUTES, \r\n" +
							 "TO_CHAR(WA.CREATED_DATE, 'DD-MM-YYYY') AS START_DATE, TO_CHAR(WA.CREATED_DATE, 'HH24:MI:SS') AS START_TIME, \r\n" +
							 "TO_CHAR(WA.TRANSACTION_DATE, 'DD-MM-YYYY') AS END_DATE, TO_CHAR(WA.TRANSACTION_DATE, 'HH24:MI:SS') AS END_TIME, \r\n" +
							 "WA.WO_ACTUAL_TRANSACTION, WA.SKILL, CASE WHEN WA.STATUS = 'CLOSED' THEN 'X' ELSE '' END AS STATUS_INDICATOR, \r\n" +
							 "W.WO, WA.TASK_CARD FROM WO W INNER JOIN WO_ACTUALS WA ON W.WO = WA.WO INNER JOIN WO_TASK_CARD_ITEM WTI ON \r\n" +
							 "WA.WO = WTI.WO AND WA.TASK_CARD = WTI.TASK_CARD WHERE W.RFO_NO IS NOT NULL AND WA.INTERFACE_ESD_TRANSFERRED_FLAG IS NULL \r\n" +
							 "AND WA.INTERFACE_ESD_TRANSFERRED_DATE IS NULL";
		 ...
		 try {
			...
			if(rs1 != null) {
				while(rs1.next()) {
					...
					pstmt2.setString(1, rs1.getString(12));
					pstmt2.setString(2, rs1.getString(13));
					...
					pstmt3.setString(1, rs1.getString(12));
					pstmt3.setString(2, rs1.getString(13));
					pstmt3.executeQuery();
