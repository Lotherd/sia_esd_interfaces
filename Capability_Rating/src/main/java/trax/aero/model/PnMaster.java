...
public class PnMaster implements Serializable {
	...
	//bi-directional many-to-one association to PnInterchangeable
	@OneToMany(mappedBy="pnMaster")
	private List<PnInterchangeable> pnInterchangeables;

	//bi-directional many-to-one association to PnInventoryLevel
	@OneToMany(mappedBy="pnMaster")
	private List<PnInventoryLevel> pnInventoryLevels;
	...
	public List<PnInterchangeable> getPnInterchangeables() {
		return this.pnInterchangeables;
	}

	public void setPnInterchangeables(List<PnInterchangeable> pnInterchangeables) {
		this.pnInterchangeables = pnInterchangeables;
	}

	public PnInterchangeable addPnInterchangeable(PnInterchangeable pnInterchangeable) {
		getPnInterchangeables().add(pnInterchangeable);
		pnInterchangeable.setPnMaster(this);

		return pnInterchangeable;
	}

	public PnInterchangeable removePnInterchangeable(PnInterchangeable pnInterchangeable) {
		getPnInterchangeables().remove(pnInterchangeable);
		pnInterchangeable.setPnMaster(null);

		return pnInterchangeable;
	}

	public List<PnInventoryLevel> getPnInventoryLevels() {
		return this.pnInventoryLevels;
	}

	public void setPnInventoryLevels(List<PnInventoryLevel> pnInventoryLevels) {
		this.pnInventoryLevels = pnInventoryLevels;
	}

	public PnInventoryLevel addPnInventoryLevel(PnInventoryLevel pnInventoryLevel) {
		getPnInventoryLevels().add(pnInventoryLevel);
		pnInventoryLevel.setPnMaster(this);

		return pnInventoryLevel;
	}

	public PnInventoryLevel removePnInventoryLevel(PnInventoryLevel pnInventoryLevel) {
		getPnInventoryLevels().remove(pnInventoryLevel);
		pnInventoryLevel.setPnMaster(null);
