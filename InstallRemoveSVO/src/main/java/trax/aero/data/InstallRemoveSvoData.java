...
public class InstallRemoveSvoData implements IInstallRemoveSvoData {
	...
	public String markTransaction(I19_Response request) throws Exception
	{
		...
		String sqlDate =
		"UPDATE PN_INVENTORY_HISTORY SET STATUS = 'CLOSED', INTERFACE_TRANSFER_FLAG = 'X', PN_INVENTORY_HISTORY.SVO_NO = ? WHERE PN_INVENTORY_HISTORY.TRANSACTION_NO = ?";
	...
	public ArrayList<I19_Request> getTransactions() throws Exception
	{
		...
		String sql= "SELECT DISTINCT A3.PN AS PN, " +
                ...
                "(select distinct ec.reference from employee_control ec, pn_master pm where ec.employee = A3.modified_by AND pm.pn = A4.PN AND ec.licence_type = pm.pn_type FETCH FIRST 1 ROWS ONLY)  AS LICENCE_TYPE, " +
                ...
                "(select dbms_lob.substr(n.notes_text, 4000, 1) from note_pad n where n.notes = A3.notes and notes_line = (SELECT MAX(notes_line) FROM note_pad WHERE notes = A3.notes)) AS NOTES, " +
